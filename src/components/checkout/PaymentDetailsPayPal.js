...
import {
	...
	PayPalCardFieldsForm, // ✅ your version exports this
	PayPalNameField,
	PayPalNumberField,
	PayPalExpiryField,
	PayPalCVVField,
	usePayPalCardFields, // used for custom submit
...
export default function PaymentDetailsPayPal(props) {
	const {
		...
		onPayApproved, // REQUIRED for paid flows
	} = props;
	...
	console.log(allowInteract, Number(selectedUsdAmount), "HOwa");
	...
	const PayArea = () => {
		...
		const createOrder = async (data, actions) => {
			...
			// Use "default" to match your patch helper
			...
			// Card Fields flow — no actions; create on the server and return order id
			...
			<>
				...
				<CardBox dir={isArabic ? "rtl" : "ltr"} aria-disabled={!allowInteract}>
					...
					>
						...
						{/* ✅ custom submit button OUTSIDE the Form */}
...
function CardSubmit({ allowInteract }) {
	...
	// Enable only when PayPal Card Fields give us a submit function (eligible + mounted)
	useEffect(() => {
		let cancelled = false;
		let attempts = 0;
		const tick = () => {
			...
			const eligible = cardFieldsForm?.isEligible?.() ?? true; // some versions expose isEligible()
	...
	const submit = async () => {
		...
		setBusy(true);
		try {
			// Optional—but nice UX: check validity first
			...
			await cardFieldsForm.submit(); // triggers 3DS if needed → then your onApprove runs
		} catch (e) {
	...
	console.log(
		"ready?",
		typeof cardFieldsForm?.submit,
		cardFieldsForm?.isEligible?.()
	);
